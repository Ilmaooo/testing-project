...
test("Filtering products on fontele.ba", async ({ page }) => {
  const filteringProducts = new filterLowestPrice(page);
  ...
  await expect(
    page.locator('.dropdown-item[data-sort="price"][data-order="asc"]')
  ).toBeVisible();

  // Step 7- Click on the “Cijena: od najniže prema najvišoj” button
